...
# Use the default Flask-Dance callback route
...
def get_alerts():
    try:
        ...
        result = []  # Fixed: properly initialized list
        for alert in alerts:
    ...
    try:
        if not ALPACA_API_KEY or not ALPACA_SECRET_KEY:
            ...
            return {
                ...
                "account": {},
                "positions": [],  # Fixed: properly initialized list
                "recent_orders": []  # Fixed: properly initialized list
            }
        ...
        try:
            ...
            try:
                ...
                positions_list = []  # Fixed: properly initialized list
                for position in positions:
                    pos_dict = position._raw if hasattr(position, '_raw') else vars(position)
                    positions_list.append(pos_dict)

                orders_list = []  # Fixed: properly initialized list
                for order in orders:
...
def get_trades():
    try:
        ...
        return jsonify({
            "trades": alpaca_data.get("recent_orders", [])
        })
    except Exception as e:
...
def get_discord_trades():
    try:
        ...
        logger.error(f"Error fetching discord trades: {str(e)}")
        return jsonify({"error": str(e)}), 500
...
def handle_connect():
    ...
    try:
        ...
        alert_list = []  # Fixed: properly initialized list
        for alert in alerts:
            try:
